...
# ========================
# ESG & Market Volatility Dashboard - Código robusto y autodepurativo
# ========================
import streamlit as st
import pandas as pd
...
CATEGORICAL_RULES_PATH = "/workspaces/ESG_marketvolatility/categorical_rules.json"
...
def print_debug(X, context):
    with open("debug_features.txt", "a") as f:
        ...
        if model_features and len(TRAINING_FEATURES) == len(model_features):
            f.write("\nMAPEO TRAINING_FEATURES -> model.feature_name_:\n")
            for i, (tf, mf) in enumerate(zip(TRAINING_FEATURES, model_features)):
                f.write(f"{i}: {tf} -> {mf}\n")
        elif model_features:
            f.write(f"\n[ALERTA] Diferente número de columnas: {len(TRAINING_FEATURES)} vs {len(model_features)}\n")
...
esg_min, esg_max = st.sidebar.slider(
    "ESG Score Range:",
    float(data["ESG Score"].min()), float(data["ESG Score"].max()),
    (0.0, 100.0)
)
vol_min, vol_max = st.sidebar.slider(
    "Volatility Range:",
    float(data["Daily_Volatility"].min()), float(data["Daily_Volatility"].max()),
    (0.0, 0.1)
)
